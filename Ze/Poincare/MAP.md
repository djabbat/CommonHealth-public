# Poincaré — MAP (Ecosystem Connections)

---

## Ecosystem Map

```
Poincare (mathematical philosophy + Ze math)
 |
 |--[Ze-intuition model]---> Ze (~/Desktop/Ze/)
 | Poincaré's discovery → Article: "Poincaré and Ze-theory:
 | process as Ze-stream Intuition as Ze-Stream" (Phase 3A ✅)
 | at v* ≈ 0.456 → Ze-EEG paper connection (planned)
 | Illumination = T-burst
 |
 |--[math problems]----------> Articles/
 | Phase 3B: 7 open math → Ze_BornRule, Ze_vstar, Ze_Foundations,
 | problems via Ze-apparatus Ze_Martingale, Ze_Collatz, Ze_Riemann,
 | Ze_NavierStokes
 |
 |--[publication]-----------> longevity.ge (Longevity Horizon)
 | Tkemaladze (2026) ×3 → (QM framework)
 | Phase 3A article ✅ (math formalism)
 | (Minkowski from Ze)
 |
 |--[cognitive model]-------> CDATA / AIM
 Poincaré's discovery → Ze-cognitive model in diagnosis_engine
 psychology → "cognitive Ze blindness" module (planned)
```

---

## Internal Research Map

```
Sources/
 01_Chronologie.md ✅ → feeds PARAMETERS.md (chronology)
 02_Intuitions_Clés.md ✅ → feeds KNOWLEDGE.md (5 intuitions)
 03_Science_et_Methode.md ✅ → Phase 2 deep analysis
 04_Carte_des_Connexions.md ✅ → feeds this MAP.md

Articles/
 Ze_BornRule_v3.md ✅ (2 review cycles)
 Ze_vstar_v3.md ✅ (2 review cycles)
 Ze_Foundations_Math_v3.md ✅ (2 review cycles)
 Ze_Martingale_v2+.md 🟡 (in progress)
 Ze_Collatz_v5.md ✅
 Ze_Riemann_v5.md ✅
 Ze_NavierStokes_v5.md ✅
 Ze_Postulates_v8.md 🟡 (peer review cycle ongoing)

scripts/
 ze_pipeline.py → automated write→review→revise pipeline
```

---

## Cross-Reference: Poincaré's Areas and Their Mathematical Descendants

| Poincaré's discovery | Modern field | Ze connection |
|---------------------|-------------|---------------|
| Qualitative ODE theory | Dynamical systems | Ze stream IS a discrete dynamical system |
| Analysis Situs | Algebraic topology | Ze impedance topology (planned) |
| Homoclinic structure / chaos | Chaos theory | v oscillation in chaotic Ze systems |
| Lorentz group | Special relativity | Ze derives Lorentz group as automorphism |
| Philosophical conventionalism | Philosophy of science | Ze: laws emerge from counting conventions |
| Mathematical discovery (subconscious) | Cognitive science | Ze model of intuition (Phase 3A article) |

---

## Ze Project (~/Desktop/Ze/) — Key Connection

Ze Vectors Theory (formerly ZeAnastasis, renamed to Ze) is the parent theoretical project.
Poincare applies Ze-formalism historically and mathematically:

- **Historical application:** Poincaré's discoveries modeled as Ze-stream dynamics
- **Mathematical application (Phase 3B):** Ze-apparatus applied to 7 open problems
- **Shared parameters:** v* ≈ 0.456, τ_Z, T-burst, Strategy A/B
- **Ze published papers:** see PARAMETERS.md → самоцитирование block

---

*Обновлено: 2026-04-03*
