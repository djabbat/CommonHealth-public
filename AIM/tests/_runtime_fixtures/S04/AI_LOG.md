
## 2026-04-26 14:32:47 — triage by scenario

**Alternatives considered:** 2

- ⭐ `glucose_ph` (test): U=0.785 (𝒞=0.10, Φ_Ze=0.99, Ethics=0.92) — Глюкоза + газы + кетоны
- ❌ `wait_morning` (wait): Laws FAIL (L1 inaction harm: impedance=0.50, red_flags=['polyuria, polydipsia, abdominal pain, ketotic breath']) — Утром посмотреть снова

**Decision:** `glucose_ph` — Глюкоза + газы + кетоны
- 𝓘: 0.26 → 0.16 (expected)

---

## 2026-04-26 14:34:03 — triage by scenario

**Alternatives considered:** 2

- ⭐ `glucose_ph` (test): U=0.785 (𝒞=0.10, Φ_Ze=0.99, Ethics=0.92) — Глюкоза + газы + кетоны
- ❌ `wait_morning` (wait): Laws FAIL (L1 inaction harm: impedance=0.50, red_flags=['polyuria, polydipsia, abdominal pain, ketotic breath']) — Утром посмотреть снова

**Decision:** `glucose_ph` — Глюкоза + газы + кетоны
- 𝓘: 0.26 → 0.16 (expected)

---

## 2026-04-26 14:39:28 — triage by scenario

**Alternatives considered:** 2

- ⭐ `glucose_ph` (test): U=0.785 (𝒞=0.10, Φ_Ze=0.99, Ethics=0.92) — Глюкоза + газы + кетоны
- ❌ `wait_morning` (wait): Laws FAIL (L1 inaction harm: impedance=0.50, red_flags=['polyuria, polydipsia, abdominal pain, ketotic breath']) — Утром посмотреть снова

**Decision:** `glucose_ph` — Глюкоза + газы + кетоны
- 𝓘: 0.26 → 0.16 (expected)

---

## 2026-04-26 14:47:08 — triage by scenario

**Alternatives considered:** 2

- ⭐ `glucose_ph` (test): U=0.785 (𝒞=0.10, Φ_Ze=0.99, Ethics=0.92) — Глюкоза + газы + кетоны
- ❌ `wait_morning` (wait): Laws FAIL (L1 inaction harm: impedance=0.50, red_flags=['polyuria, polydipsia, abdominal pain, ketotic breath']) — Утром посмотреть снова

**Decision:** `glucose_ph` — Глюкоза + газы + кетоны
- 𝓘: 0.26 → 0.16 (expected)

---

## 2026-04-30 15:26:13 — triage by scenario

**Alternatives considered:** 2

- ⭐ `glucose_ph` (test): U=0.795 (𝒞=0.16, Φ_Ze=0.99, Ethics=0.92) — Глюкоза + газы + кетоны
- ❌ `wait_morning` (wait): Laws FAIL (L1 inaction harm: impedance=0.50, red_flags=['polyuria, polydipsia, abdominal pain, ketotic breath']) — Утром посмотреть снова

**Decision:** `glucose_ph` — Глюкоза + газы + кетоны
- 𝓘: 0.41 → 0.25 (expected)

---

## 2026-04-30 21:41:51 — triage by scenario

**Alternatives considered:** 2

- ⭐ `glucose_ph` (test): U=0.784 (𝒞=0.10, Φ_Ze=0.99, Ethics=0.92) — Глюкоза + газы + кетоны
- ❌ `wait_morning` (wait): Laws FAIL (L1 inaction harm: impedance=0.50, red_flags=['polyuria, polydipsia, abdominal pain, ketotic breath']) — Утром посмотреть снова

**Decision:** `glucose_ph` — Глюкоза + газы + кетоны
- 𝓘: 0.26 → 0.16 (expected)

---
