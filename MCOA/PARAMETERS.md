...
* **TBD (placeholder — to be calibrated from literature or pilot data by 2026-07-01):** Временное значение, используемое для симуляций; требует замены на Fixed или To be calibrated.
...
## Timeline for a priori weight prediction

A systematic workflow for *a priori* weight prediction (RNA-seq + proliferation rate + metabolomics) is outlined in OPEN_PROBLEMS.md (Test 1A) and is expected to be completed by [month/year].
